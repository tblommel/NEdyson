...
int main(int argc, char *argv[]){
	...
	chrono::time_point<std::chrono::system_clock> start, end, start_tot, end_tot, start1, end1;
	...
	for(int tstp=k+1;tstp<=Nt;tstp++){
		...
		cout.precision(17);
		double npart = DensM.trace().real();
		std::cout << tstp << " number of particles = " << npart << endl;
	} // End Timestepping
	...
	cout << "Total Time = " << (end-start_tot).count() << "s\n\n";
	
	G.print_to_file(argv[1],dt,dtau);
}
