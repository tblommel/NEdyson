...
void get_bubble_1_mat(cplx* cmat, int sc, int c1, int c2, cplx *amat, int sa, int a1, int a2, cplx *bmat, int sb, int b1, int b2, int sigb, int ntau){
	int m;
...
void get_bubble_1_tstp(int tstp, cplx *cret, cplx *ctv, cplx *cles, int sc, int c1, int c2,
																 cplx *aret, cplx *atv, cplx *ales, int sa, int a1, int a2,
																 cplx *bret, cplx *btv, cplx *bles, int sb, int b1, int b2, int bsig, int ntau){
	int m;
...
void get_bubble_2_mat(cplx *smat,int ss,int s1, int s2, cplx *gmat, int sg, int g1, int g2, cplx *pmat,int sp,int p1,int p2,int ntau){
	int m;
	int es = ss*ss;
...
void get_bubble_2_tstp(int tstp, cplx *sret, cplx *stv, cplx *sles, int ss, int s1, int s2,
																 cplx *gret, cplx *gtv, cplx *gles, int gs, int g1, int g2,
																 cplx *pret, cplx *ptv, cplx *ples, int ps, int p1, int p2, int ntau){
	int m;
	int es = ss*ss;
	int count12 = s1*ss+s2;
...
void Bubble2(int tstp, GREEN &Sigma, int s1, int s2, GREEN &G, int g1, int g2, GREEN &Pol, int p1, int p2){
	int ntau = G.ntau();
	if(tstp == -1) get_bubble_2_mat(Sigma.matptr(0),Sigma.size1(),s1,s2,G.matptr(0),G.size1(),g1,g2,Pol.matptr(0),Pol.size1(),p1,p2,ntau);
	else get_bubble_2_tstp(tstp,Sigma.retptr(tstp,0),Sigma.tvptr(tstp,0),Sigma.lesptr(0,tstp),Sigma.size1(),s1,s2,
															G.retptr(tstp,0),G.tvptr(tstp,0),G.lesptr(0,tstp),G.size1(),g1,g2,
...
void Bubble1(int tstp, GREEN &Pol, int p1, int p2, GREEN_TSTP &A, int a1, int a2, GREEN &B, int b1, int b2){
	int ntau = Pol.ntau();
	if(tstp == -1) get_bubble_1_mat(Pol.matptr(0),Pol.size1(),p1,p2,A.matptr(0),A.size1(),a1,a2,B.matptr(0),B.size1(),b1,b2,B.sig(), ntau);
...
void Ham_MF(int tstp, GREEN &G, CFUNC &Ut, cdmatrix &h0, CFUNC &hmf){
	int nsites=G.size1();
...
void GenTPP(int tstp, double dt, double beta, GREEN &G, GREEN &Phi, CFUNC &Ut, GREEN &PhixU, GREEN &UxPhi, GREEN &TPP, INTEG &I){
