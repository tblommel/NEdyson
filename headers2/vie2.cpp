...
void vie2_mat_fourier(GREENS &G, GREENS &F, GREENS &Fcc, GREENS &Q, double beta, int pcf){
...
void vie2_ret_start(GREEN &G, GREEN &F, GREEN &Fcc, GREEN &Q, INTEG &I, double dt){
...
void vie2_tv_start(GREEN &G, GREEN &F, GREEN &Fcc, GREEN &Q, INTEG &I, double dt, double beta){
...
/*
	...
	// Boundary Conditions
	for(m=0;m<=ntau;m++){
		for(i=0;i<es;i++) G.tvptr(0,m)[i] = cplxi*(double)G.sig()*G.matptr(ntau-m)[i];
	}

	// Do the integrals and store them in Gtv(n,m)
	for(m=0;m<=ntau;m++){
		for(n=1;n<=k;n++){
			CTV2_vie2(I, F, G, n, m, beta, tmp);
			CTV3_vie2(I, F, G, n, m, beta, stmp);
			for(i=0;i<es;i++) G.tvptr(n,m)[i] = -tmp[i] - stmp[i];
		}
	}
	...
	for(m=0;m<=ntau;m++){
		...
		// Set up the kxk linear problem
		for(n=1;n<=k;n++){
			element_mult(size1,Y+(n-1)*es,F.retptr(n,0),G.tvptr(0,m));
			element_smul(size1,Y+(n-1)*es,-dt*I.gregory_weights(n,0));
			for(l=1;l<=k;l++){
				dnl = n==l?1.:0.;
				weight = I.gregory_weights(n,l)*dt;
				if(n>=l){ // use F
				...
				for(i=0;i<es;i++){
					M[(n-1)*k*es+(i/size1)*k*size1+(l-1)*size1+i%size1] = dnl*iden[i]+weight*tmp[i];
				}
			}
			
			// add the integrals
			element_incr(size1, Y+(n-1)*es, G.tvptr(n,m));
			
			// add Q
			element_incr(size1, Y+(n-1)*es, Q.tvptr(n,m));
		}

		// Solve MX=Y
		element_linsolve_left(k*size1,k*size1,size1,M,X,Y);

		// Put result into G
		for(l=0;l<k;l++) element_set(size1, G.tvptr(l+1,m), X+l*es);
	}
*/
	delete[] M;
	delete[] Y;
...
void vie2_les_timestep(int n, GREENS &G, GREENS &F, GREENS &Fcc, GREENS &Q, double beta, double dt, INTEG &I){
...
void vie2_les_start(GREENS &G, GREENS &F, GREENS &Fcc, GREENS &Q, INTEG &I, double dt, double beta){
	int k=I.k();
	for(int i=0;i<=k;i++){
...
// for t' = 0...n
void vie2_ret_timestep(int n, GREENS &G, GREENS &F, GREENS &Fcc, GREENS &Q, INTEG &I, double dt){
...
void vie2_tv_timestep(int n, GREENS &G, GREENS &F, GREENS &Fcc, GREENS &Phi, INTEG &I, double dt, double beta){
