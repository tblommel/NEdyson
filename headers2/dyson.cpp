...
// gives an interpolation of a function on the domain [tstp-k,tatp]
// eps(tinterp) = sum_{l=0}^k eps(tstp-k+l) * sum_{p=0}^k (tinterp+k-tstp)^p P_{pl}
cdmatrix interpolate(int tstp, double tinterp, INTEG &I, function &eps){
...
// c1=(1-sqrt(3)/3)/2
// c2=(1+sqrt(3)/3)/2
void propagator_exp(int tstp, INTEG &I, function &Ut, function &eps, double dt){
	int size = Ut.size1();
...
void G0_from_h0_disp(green_func &G, double mu, cdmatrix &H0, double beta, double h,int size){
 ...
 //NOT YET DONE FOR size=1
void G0_from_h0(green_func &G, double mu, cdmatrix &h0, double beta, double dt){
	int size=G.size1();
	if(size>1) G0_from_h0_disp(G, mu, h0, beta, dt, size);
	//else G0_from_const_h0_disp(G, mu, h0, beta, dt);
}
...
void mat_fourier(green_func &G, green_func &Sigma, double mu, cplx *h0, double beta){
...
void Extrapolate(integration::Integrator &I, green_func &G, int n){
...
void GRstart(integration::Integrator &I, green_func &G, green_func &Sig, function &hmf, double mu, double dt){
	// Counters and sizes
...
void GTVstart(INTEG &I, green_func &G, green_func &Sig, function &hmf, double mu, double beta, double dt){
	...
	for(m=0;m<=ntau;m++){
		...
		for(n=1;n<=k;n++){
			...
			element_incr(size1,Q+(n-1)*es,G.tvptr(n,m));
		}
		// Solve MX=Q
...
void GRstep(int tstp, integration::Integrator &I, green_func &G, green_func &Sig, function &hmf, double mu, double dt){
...
void dyson_start(INTEG &I, green_func &G, green_func &Sig, function &hmf, double mu, double beta, double dt){
	GRstart(I, G, Sig, hmf, mu, dt);
	GTVstart(I, G, Sig, hmf, mu, beta, dt);
	GLstart(I, G, Sig, hmf, mu, beta, dt);
}
