...
TEST_CASE("TTI Sigma HF")
{
  ...
  ZTensor<2> rho(na,na);
  ZTensor<2> hmf(na, na);
  ZTensor<2> hmf_loop(na, na);

  DColVectorMap(U_int.data(),na4) = Eigen::VectorXd::Random(na4);
  ZColVectorMap(rho.data(), na2) = Eigen::VectorXcd::Random(na2);
  ZColVectorMap(hmf.data(), na2) = Eigen::VectorXcd::Random(na2);
  hmf_loop = hmf;

  tti_molHFSolver test_solver(U_int);
  
  test_solver.solve_HF(hmf,rho);
  test_solver.solve_HF_loop(hmf_loop,rho);


  double err = (ZColVectorMap(hmf.data(), na2) - ZColVectorMap(hmf_loop.data(), na2)).norm();
  REQUIRE(err < 1e-10);
}


TEST_CASE("TTI Decomp Sigma HF")
{
  using namespace NEdyson;
  ...
  DTensor<3> Vija(nao,nao,nalpha);
  ZTensor<2> rho(nao,nao);
  ZTensor<2> hmf(nao, nao);
  ZTensor<2> hmf_loop(nao, nao);

  DColVectorMap(Vija.data(),nao2*nalpha) = Eigen::VectorXd::Random(nao2*nalpha);
  ZColVectorMap(rho.data(), nao2) = Eigen::VectorXcd::Random(nao2);
  ZColVectorMap(hmf.data(), nao2) = Eigen::VectorXcd::Random(nao2);
  hmf_loop = hmf;

  tti_molHFSolverDecomp test_solver(Vija);
  
  test_solver.solve_HF(hmf,rho);
  test_solver.solve_HF_loop(hmf_loop,rho);

  double err = (ZColVectorMap(hmf.data(), nao2) - ZColVectorMap(hmf_loop.data(), nao2)).norm();
  REQUIRE(err < 1e-10);
}


TEST_CASE("TTI Sigma HF Spin")
{
  using namespace NEdyson;
  ...
  DTensor<4> U_int(na,na,na,na);
  ZTensor<3> rho(2, na, na);
  ZTensor<3> hmf(2, na, na);
  ZTensor<3> hmf_loop(2, na, na);

  DColVectorMap(U_int.data(),na4) = Eigen::VectorXd::Random(na4);
  ZColVectorMap(rho.data(), 2*na2) = Eigen::VectorXcd::Random(2*na2);
  ZColVectorMap(hmf.data(), 2*na2) = Eigen::VectorXcd::Random(2*na2);
  hmf_loop = hmf;

  tti_molHFSolverSpin test_solver(U_int);
