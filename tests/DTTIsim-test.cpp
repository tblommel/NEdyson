...
TEST_CASE("DTTIsim Tests") {
  ...
  // Make the sim
  std::unique_ptr<tti_DecompSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<tti_DecompSimulation<gfmol::ChebyshevRepr>>(new tti_DecompSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, nw, wmax, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
  ...
  ZColVectorMap GRVec = ZColVectorMap(p_sim->G.retptr(0), (nt+1)*Gcheck.element_size());
  ZColVectorMap GRcVec = ZColVectorMap(Gcheck.retptr(0), (nt+1)*Gcheck.element_size());
  REQUIRE((GRVec-GRcVec).norm() < 1e-12);

  ZColVectorMap GLVec = ZColVectorMap(p_sim->G.lesptr(0), (nt+1)*Gcheck.element_size());
  ZColVectorMap GLcVec = ZColVectorMap(Gcheck.lesptr(0), (nt+1)*Gcheck.element_size());
  ...
  DColVectorMap AVec = DColVectorMap(p_sim->A.ptr(0), Acheck.nw()*Acheck.es());
  DColVectorMap AcVec = DColVectorMap(Acheck.ptr(0), Acheck.nw()*Acheck.es());
