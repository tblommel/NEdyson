...
TEST_CASE("DTTIsim Tests") {
  ...
  // Make the sim
  std::unique_ptr<tti_DecompSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<tti_DecompSimulation<gfmol::ChebyshevRepr>>(new tti_DecompSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
