...
TEST_CASE("Ssim Tests") {
  ...
  h5e::File check_file(std::string(TEST_DATA_DIR) + "/Ssim.h5");
  GREEN Gdowncheck;
  GREEN Gupcheck;
  SPECT Acheck;
  ...
  // Make the sim
  std::unique_ptr<SpinSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<SpinSimulation<gfmol::ChebyshevRepr>>(new SpinSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, nw, wmax, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
