...
TEST_CASE("Ssim Tests") {
  ...
  // Make the sim
  std::unique_ptr<SpinSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<SpinSimulation<gfmol::ChebyshevRepr>>(new SpinSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
