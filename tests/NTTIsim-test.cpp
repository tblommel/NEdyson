...
TEST_CASE("NTTIsim Tests") {
  ...
  // Make the sim
  std::unique_ptr<tti_Simulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<tti_Simulation<gfmol::ChebyshevRepr>>(new tti_Simulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, nw, wmax, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
