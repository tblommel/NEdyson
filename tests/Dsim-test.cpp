...
TEST_CASE("Dsim Tests") {
  ...
  // Make the sim
  std::unique_ptr<DecompSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<DecompSimulation<gfmol::ChebyshevRepr>>(new DecompSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
