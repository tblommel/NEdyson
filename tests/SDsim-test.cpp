...
TEST_CASE("SDsim Tests") {
  ...
  h5e::File check_file(std::string(TEST_DATA_DIR) + "/SDsim.h5");
  GREEN Gdowncheck;
  GREEN Gupcheck;
  SPECT Acheck;
  ...
  // Make the sim
  std::unique_ptr<DecompSpinSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<DecompSpinSimulation<gfmol::ChebyshevRepr>>(new DecompSpinSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, nw, wmax, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
  ...
  ZColVectorMap GuRVec = ZColVectorMap(p_sim->G[0].get().retptr(0,0), (nt+1)*(nt+2)/2*es);
  ZColVectorMap GuRcVec = ZColVectorMap(Gupcheck.retptr(0,0), (nt+1)*(nt+2)/2*es);
  REQUIRE((GuRVec-GuRcVec).norm() < 1e-12);

  ZColVectorMap GuLVec = ZColVectorMap(p_sim->G[0].get().lesptr(0,0), (nt+1)*(nt+2)/2*es);
  ZColVectorMap GuLcVec = ZColVectorMap(Gupcheck.lesptr(0,0), (nt+1)*(nt+2)/2*es);
  ...
  DColVectorMap AuVec = DColVectorMap(p_sim->A.ptr(0,0), (Acheck.nt()+1)*Acheck.nw()*es);
  DColVectorMap AucVec = DColVectorMap(Acheck.ptr(0,0), (Acheck.nt()+1)*Acheck.nw()*es);
  ...
  ZColVectorMap GdRVec = ZColVectorMap(p_sim->G[1].get().retptr(0,0), (nt+1)*(nt+2)/2*es);
  ZColVectorMap GdRcVec = ZColVectorMap(Gdowncheck.retptr(0,0), (nt+1)*(nt+2)/2*es);
  REQUIRE((GdRVec-GdRcVec).norm() < 1e-12);

  ZColVectorMap GdLVec = ZColVectorMap(p_sim->G[1].get().lesptr(0,0), (nt+1)*(nt+2)/2*es);
  ZColVectorMap GdLcVec = ZColVectorMap(Gdowncheck.lesptr(0,0), (nt+1)*(nt+2)/2*es);
