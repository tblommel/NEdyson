  ...
  for(int i=0; i<nao; i++){
    for(int j=0; j<nao; j++){
      for(int a=0; a<nalpha; a++){
        Viaj(i,a,j) = Vija(i,j,a);
      }
    }
  }

  molGF2SolverSpinDecomp test_solver(Vija, Viaj);
...
TEST_CASE("Decomp GF2")
{
  ...
  molGF2SolverDecomp test_solver(Vija, Viaj);
...
TEST_CASE("Spin GF2")
{
  ...
  DTensor<4> U_exch(nao,nao,nao,nao);
  ...
  for (int j = 0; j < nao; ++j) {
    for (int l = 0; l < nao; ++l) {
      for (int n = 0; n < nao; ++n) {
        for (int q = 0; q < nao; ++q) {
          U_exch(j, n, l, q) = Uint(j, l, n, q);
        }
      }
    }
  }

  molGF2SolverSpin test_solver(Uint, U_exch);
...
TEST_CASE("Full GF2")
{
  ...
  DTensor<4> Uijkl_exch(nao,nao,nao,nao);
  ...
  for (int j = 0; j < nao; ++j) {
    for (int l = 0; l < nao; ++l) {
      for (int n = 0; n < nao; ++n) {
        for (int q = 0; q < nao; ++q) {
          Uijkl_exch(j, l, n, q) = 2 * Uijkl(j, n, l, q) - Uijkl(j, l, n, q);
        }
      }
    }
  }
  ...
  molGF2Solver test_solver(Uijkl, Uijkl_exch);
...
TEST_CASE("TTI Decomp Spin GF2")
{
  ...
  tti_molGF2SolverSpinDecomp test_solver(Vija, Viaj);
...
TEST_CASE("TTI Decomp GF2")
{
  ...
  tti_molGF2SolverDecomp test_solver(Vija, Viaj);
...
TEST_CASE("TTI Spin GF2")
{
  ...
  tti_molGF2SolverSpin test_solver(Uint, U_exch);
...
TEST_CASE("TTI Full GF2")
{
  ...
  for (int j = 0; j < nao; ++j) {
    for (int l = 0; l < nao; ++l) {
      for (int n = 0; n < nao; ++n) {
        for (int q = 0; q < nao; ++q) {
          U_exch(j, l, n, q) = 2 * Uijkl(j, n, l, q) - Uijkl(j, l, n, q);
        }
      }
    }
  }

  tti_molGF2Solver test_solver(Uijkl, U_exch);
