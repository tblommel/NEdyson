...
TEST_CASE("SDTTIsim Tests") {
  ...
  // Make the sim
  std::unique_ptr<tti_DecompSpinSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<tti_DecompSpinSimulation<gfmol::ChebyshevRepr>>(new tti_DecompSpinSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
