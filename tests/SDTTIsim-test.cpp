...
TEST_CASE("SDTTIsim Tests") {
  ...
  h5e::File check_file(std::string(TEST_DATA_DIR) + "/SDTTIsim.h5");
  ...
  // Make the sim
  std::unique_ptr<tti_DecompSpinSimulation<gfmol::ChebyshevRepr>> p_sim = std::unique_ptr<tti_DecompSpinSimulation<gfmol::ChebyshevRepr>>(new tti_DecompSpinSimulation<gfmol::ChebyshevRepr>(hf, frepr, brepr, nt, ntau, k, dt, nw, wmax, MatMaxIter, MatMaxErr, BootMaxIter, BootMaxErr, CorrSteps, mode, 0));
