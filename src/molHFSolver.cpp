...
void tti_molHFSolver::solve_HF_loop(ZTensor<2> &hmf, const ZTensor<2> &rho) const {
  assert(hmf.shape()[0] == nao_);
  assert(hmf.shape()[1] == nao_);
  assert(rho.shape()[0] == nao_);
  assert(rho.shape()[1] == nao_);

  ZMatrixMap hmfMap = ZMatrixMap(hmf.data(),nao_,nao_);
...
void tti_molHFSolver::solve_HF(ZTensor<2> &hmf, const ZTensor<2> &rho) const {
  assert(hmf.shape()[0] == nao_);
  assert(hmf.shape()[1] == nao_);
  assert(rho.shape()[0] == nao_);
  assert(rho.shape()[1] == nao_);
  ...
  for(int i=0; i<nao_; ++i){
    DMatrixConstMap U_i_lk_j = DMatrixConstMap(Uijkl_.data() + i*nao3, nao2, nao_);
    ZColVectorMap(hmf.data() + i*nao_, nao_).noalias() -= rho_T_flat * U_i_lk_j;
  }
  
  ZColVectorMap(hmf.data(), nao2).noalias() += 2*DMatrixConstMap(Uijkl_.data(),nao2,nao2) * ZColVectorConstMap(rho.data(), nao2);
}

void tti_molHFSolverDecomp::solve_HF_loop(ZTensor<2> &hmf, const ZTensor<2> &rho) const {
  assert(hmf.shape()[1] == nao_);
  assert(hmf.shape()[0] == nao_);
  assert(rho.shape()[1] == nao_);
  assert(rho.shape()[0] == nao_);
...
void tti_molHFSolverDecomp::solve_HF(ZTensor<2> &hmf, const ZTensor<2> &rho) const {
  assert(hmf.shape()[1] == nao_);
  assert(hmf.shape()[0] == nao_);
  assert(rho.shape()[1] == nao_);
  assert(rho.shape()[0] == nao_);
  ...
  auto RhomapI = ZRowVectorConstMap(rho.data(), nao2);
  Xa_ = RhomapI * VmapIa;
  auto hmfmapI = ZRowVectorMap(hmf.data(), nao2);
  hmfmapI.noalias() += 2 * Xa_ * VmapIa.transpose();
  ...
  for(int i=0;i<nao_;i++){
    auto V_i_l_a = DMatrixConstMap(Vija_.data() + i*nao_*nalpha_, nao_, nalpha_);
    tmp_1_23 = V_i_l_a * V_kj_a_T;
    ZRowVectorMap(hmf.data() + i*nao_, nao_).noalias() -= rho_T_flat * tmp_12_3;
  }
}

void tti_molHFSolverSpin::solve_HF_loop(ZTensor<3> &hmf, const ZTensor<3> &rho) const {
  assert(hmf.shape()[0] == 2);
  assert(hmf.shape()[1] == nao_);
  assert(hmf.shape()[2] == nao_);
  assert(rho.shape()[0] == 2);
  ...
  for(int s=0; s<ns_; ++s){
    for(int i=0; i<nao_; ++i){
      for(int j=0; j<nao_; ++j){
        for(int k=0; k<nao_; ++k){
          for(int l=0; l<nao_; ++l){
            hmf(s,i,j) -= Uijkl_(i,l,k,j) * rho(s, k, l);
            for(int sp=0; sp<ns_; sp++){
              hmf(s,i,j) += rho(sp,k,l) * Uijkl_(i,j,k,l);
            }
          }
        }
      }
...
void tti_molHFSolverSpin::solve_HF(ZTensor<3> &hmf, const ZTensor<3> &rho) const {
  assert(hmf.shape()[0] == 2);
  assert(hmf.shape()[1] == nao_);
  assert(hmf.shape()[2] == nao_);
  assert(rho.shape()[0] == 2);
  ...
  for(int s=0; s<ns_; ++s){
    ...
    for(int i=0; i<nao_; ++i){
      DMatrixConstMap Ui_lk_j(Uijkl_.data() + i*nao3, nao2, nao_);
      ZColVectorMap(hmf.data() + s*nao2 + i*nao_, nao_).noalias() -= rho_T_flat * Ui_lk_j;
    }

    // Hartree
    for(int sp=0; sp<ns_; sp++){
      ZColVectorMap(hmf.data() + s*nao2, nao2).noalias() += DMatrixConstMap(Uijkl_.data(),nao2,nao2) * ZColVectorConstMap(rho.data() + sp*nao2 ,nao2);
    }
  }
}
