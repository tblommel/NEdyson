...
int main(const int argc, char *const *const argv)
{
  ...
  if (p.repr == "cheb") {
    ...
    std::unique_ptr<SimulationBase> p_sim =
        make_simulation(p.tti, p.unrestricted, p.decomposed, hf, frepr, brepr, mode, p.nt, p.ntau, p.k, p.dt, p.maxiter, p.etol, p.BootMaxIter, p.BootMaxErr, p.CorrSteps, p.damping, p.decomp_prec, p.hf);
